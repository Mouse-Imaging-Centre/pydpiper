...
```{bash MBM.py, eval=(params$pipeline=="MBM" & params$run_pipeline), include=params$pipeline=="MBM"}
#Run `tail -F MBM/MBM_out.txt` to track the standard output
...
--init-model test-data_20180925/Pydpiper-40-micron-basket-dec-2014/basket_mouse_brain_40micron.mnc \
\
--run-maget \
...
```{bash twolevel_model_building.py, eval=(params$pipeline=="twolevel_model_building" & params$run_pipeline), include=params$pipeline=="twolevel_model_building"}
#Run `tail -F twolevel_model_building/twolevel_model_building_out.txt` to track the standard output
twolevel_model_building.py --pipeline-name=twolevel_test \
--subject-matter mousebrain \
--num-executors 1000 --time 48:00:00 \
--csv-file test-data_20180925/input2.csv \
--lsq6-large-rotations-tmp-dir=/tmp \
--init-model test-data_20180925/Pydpiper-40-micron-basket-dec-2014/basket_mouse_brain_40micron.mnc \
\
--run-maget \
...
\
--lsq12-protocol test-data_20180925/Pydpiper_testing_default_lsq12.csv \
>> twolevel_model_building_out.txt
```
