...
class SMATregister:
    ...
    def __init__(self, input, template, outDir="",
                 steps=[0.5,0.2],
        ...
        # nu_correct the input
        inuc, inuc_log = fh.createOutputAndLogFiles(input_base, log_base, "_nuc.mnc" )
        p.addStage(nu_correct(input, inuc, inuc_log))

        #
        # log_dir, input_file, template
        # nuc_out = p.append(nu_correct(input_file, log_dir))
        # mritotal_out = p.append(mritotal(nuc_out, template, log_dir))

        # linear registration to TAL space, using the template as a model
        
        linxfm, logfile = fh.createXfmAndLogFiles(input_base, log_base, ".xfm")
